...
https://github.com/Azure/azure-content/blob/master/articles/cloud-services/cloud-services-dotnet-install-dotnet.md
...
This article describes how to install .NET framework on Cloud Service Web and Worker Roles. You can use these steps to install .NET 4.6.1 on the Azure Guest OS Family 4. For the latest information on Guest OS releases see Azure Guest OS Releases and SDK Compatibility Matrix.
...
For a Worker Role
Right click on your role and select Add>Existing Item. Select the .NET installer and add it to the role.
...
Note:
You should install .NET 4.6.1 on your Cloud Service role even if your application targets .NET 4.6. The Azure Guest OS includes updates 3098779 and 3097997. Installing .NET 4.6 on top of these updates may cause issues when running your .NET applications, so you should directly install .NET 4.6.1 instead of .NET 4.6. For more information, see KB 3118750.

Role Contents with installer files

Define startup tasks for your roles
Startup tasks allow you to perform operations before a role starts. Installing the .NET Framework as part of the startup task will ensure that the framework is installed before any of your application code is run. For more information on startup tasks see: Run Startup Tasks in Azure.

Add the following to the ServiceDefinition.csdef file under the WebRole or WorkerRole node for all roles:
...
Add the following script to the install.cmd file:
...
Note:
The script still shows you how to install .NET 4.5.2 or .NET 4.6 for continuity. There is no need to install .NET 4.5.2 manually as it is already available on Azure Guest OS. Instead of installing .NET 4.6 you should directly install .NET 4.6.1 due to KB 3118750.
